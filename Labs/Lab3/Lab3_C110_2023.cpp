...
BOOK gbook; //глобальная структура
...
//в заголовочный файл.
	...
	static BOOK sbook; //статическая структура
	BOOK lbook; //локальная структура
	BOOK* dbook = new BOOK(); //динамическая структура
	...
	std::cout << "Global size: " << sizeof(gbook) << std::endl;
	std::cout << "Static size: " << sizeof(sbook) << std::endl;
	std::cout << "Local size: " << sizeof(lbook) << std::endl;
	std::cout << "Dynamic size: " << sizeof(dbook) << std::endl;
	...
	strcpy_s(lbook.author, AUTHOR_SZ, "Kip Stephen Thorne");
	strcpy_s(lbook.title, TITLE_SZ, "The Science of Interstellar");
	lbook.year = 2014;
	lbook.price = 15.78;
	lbook.subject = Physics;
...
//Задание 3 (*).
#if 0
	//Предоставьте пользователю возможность выводить перечень книг
